...
def keluarga(row: dict, data: dict | None = None) -> str:
    """
    Tentukan varian crop sebuah run: "asli" atau "eq48".

    Semula ini ditebak dari nama berkas registry (`"eq_dev" in nama`). Tebakan
    itu benar untuk dua registry pertama tapi DIAM-DIAM salah untuk nama lain:
    `pio_dev2_eq_registry.json` tidak memuat "eq_dev", jadi lengan eq48-nya
    akan dicap "asli" dan dirata-ratakan bersama lengan yang lain - dua
    susunan data berbeda tercampur dalam satu baris tabel, tanpa peringatan.

    Sekarang dibaca dari `config_snapshot.crops.equalize_resolution` di
    registry itu sendiri, yaitu sumber yang sama yang dipakai saat crop dibuat.
    Nama berkas tidak lagi ikut menentukan apa pun.
    """
    eq = ((row.get("config_snapshot") or {})
          .get("crops", {}).get("equalize_resolution") or {})
    if not eq:
        # Rekaman lama (dan JSON yang sudah ter-commit) tidak menyimpan
